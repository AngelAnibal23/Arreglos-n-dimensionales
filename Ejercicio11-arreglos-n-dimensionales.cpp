...
void inicializarBosque(int bosque[N][N]) {
    srand(time(0));
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            bosque[i][j] = VIVO;
    ...
    bosque[rand() % N][rand() % N] = QUEMANDOSE;
}
...
void propagarFuego(int bosque[N][N], int nuevoBosque[N][N]) {
        ...
        for (int j = 0; j < N; ++j) {
            if (bosque[i][j] == VIVO) {
                ...
                for (int k = 0; k < 8; ++k) {
                    int ni = i + dx[k];
                    int nj = j + dy[k];
                    if (ni >= 0 && ni < N && nj >= 0 && nj < N && bosque[ni][nj] == QUEMANDOSE) {
...
int main() {
    ...
    cout << "Estado del bosque después de la propagación del fuego:" << endl;
    mostrarBosque(bosque);
